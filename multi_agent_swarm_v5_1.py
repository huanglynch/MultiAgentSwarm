...
# ====================== 🔥 图片预压缩（解决Token爆炸核心） ======================
def compress_image_for_vision(image_path: str, max_side: int = 800, quality: int = 75) -> str:
    """
    专为多模态LLM优化图片大小
...
class Agent:
    ...
    def _execute_tool(self, tool_call) -> Dict:
        ...
        try:
            ...
            if len(result_str) > 2800:
                result_str = result_str[:2800] + "\n\n...(已智能截断)\n💡 建议：如需完整结果，请简化参数或分步执行。"
...
class MultiAgentSwarm:
    ...
    def create_echo_agent(self, thinking_mode: str, mode_prompt: str) -> Agent:
        """动态创建Echo自由Agent（零固定角色）"""
        echo_config = {
            "name": f"Echo_{thinking_mode.upper()}",
            "role": f"【Echo自由Agent】当前任务最优认知模式：{thinking_mode}\n\n{mode_prompt}",
            "api_key": self.agents[0].client.api_key,
            "base_url": self.agents[0].client.base_url,
            "model": self.default_model,
            "temperature": 0.75,      # 略高一点，让它更灵活
            "stream": False,
            "max_tokens": 4096,
            "enabled_tools": ["web_search", "browse_page", "code_executor"]  # 可按需扩展
        }
        agent = Agent(
            echo_config,
            self.default_model,
        ...
        self.temporary_agents.append(agent)
        logging.info(f"🧬 Echo自由Agent已激活 → 模式: {thinking_mode}")
        return agent
    ...
    def _verify_claims(self, history: List[Dict], final_answer: str) -> Tuple[int, str]:
        """极简Verifier：自动提取Claims → 并行fact-check → 返回质量分+建议
        复用现有工具 + ThreadPoolExecutor，零新依赖"""
        logging.info("🔍 Verifier启动：并行事实核查")

        # 1. 提取Claims（让Leader快速拆解）
        extract_prompt = (
            f"请从以下最终答案中提取3-5条核心Claims（事实性陈述），用编号列表返回，不要解释：\n{final_answer[:1500]}"
        )
        claims_text = self.leader.generate_response(
            [{"speaker": "System", "content": extract_prompt}], 0, force_non_stream=True
        )
        claims = [line.strip() for line in claims_text.split("\n") if line.strip() and line[0].isdigit()][:5]

        # 2. 并行fact-check（每个Claim调用web_search）
        results = []
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_to_claim = {
                executor.submit(web_search, f"事实核查: {claim}", num_results=3): claim
                for claim in claims
            }
            for future in as_completed(future_to_claim):
                ...
                try:
                    ...
                    results.append(f"Claim: {claim}\n搜索验证: {search_result[:300]}...")
                except:
                    results.append(f"Claim: {claim}\n搜索失败")

        # 3. Meta-Critic打分（复用现有逻辑）
        verify_prompt = (
            f"Claims核查结果：\n" + "\n".join(results) +
            f"\n\n最终答案：{final_answer[:800]}\n\n"
            "请给出最终质量分（0-100）并简述是否需要Re-Plan/Re-Debate（JSON格式）。"
        )
        score_json = self.leader.generate_response(
            history + [{"speaker": "System", "content": verify_prompt}], 0, force_non_stream=True
        )
        try:
            data = json.loads(score_json.strip().replace("```json", "").replace("```", ""))
            score = data.get("quality_score", 85)
            decision = data.get("need_replan", "no")
    ...
    def solve(
        ...
        try:
            ...
            # 🔥【修复版】智能路由 + Echo 模式解包（只需改这里）
            if self.intelligent_routing_enabled:
                if force_complexity is not None:
                    complexity = force_complexity
                elif self.force_complexity is not None:
                    complexity = self.force_complexity
                else:
                    # ← 关键修复：正确解包三元组
                    category, thinking_mode, mode_prompt = self._classify_task_complexity(classification_task)
                    complexity = category

                    # 🔥 新增：保存给 Echo 使用，避免重复调用
                    self._echo_thinking_mode = thinking_mode
                    self._echo_mode_prompt = mode_prompt

                # === 🔥 WebUI 自动降级保护（保留你原有逻辑）===
    ...
    def _solve_complex(
        ...
        # 🔥 Echo自由Agent激活（使用 solve() 已计算的结果，避免重复调用）
        thinking_mode = getattr(self, '_echo_thinking_mode', 'first_principles')
        mode_prompt = getattr(self, '_echo_mode_prompt', '你现在进入第一性原理拆解模式。')
        echo = self.create_echo_agent(thinking_mode, mode_prompt)
        selected_agents = self.agents[:3] + [echo]  # 3个固定 + Echo
        ...
        # 🔥 最终Verifier（强制！必须在答案生成之后）
        if self.enable_adversarial_debate:
            final_quality, _ = self._verify_claims(history, final_answer)
            logging.info(f"🔍 Verifier最终评分: {final_quality}/100")
            if final_quality < 85:
                logging.warning(f"最终答案质量仅 {final_quality}，已触发额外辩论")
                self._adversarial_debate(history, round_num + 1)
                # 可选：质量低时再生成一次更高质量答案
                final_answer = self.leader.generate_response(
                    history, round_num + 2,
                    force_non_stream=False,
        ...
        tracker.checkpoint("5️⃣ 最终综合 + Verifier")
    ...
    def _classify_task_complexity(self, task: str) -> Tuple[str, str, str]:
        ...
        if any(kw in task_lower or kw in task for kw in force_complex):
            return "complex", "real_time_tracking", "你现在进入实时追踪+多源交叉验证模式。"
        ...
        if score["balanced"] >= 4:
            return "balanced", "first_principles", "你现在进入第一性原理拆解模式：从原子事实开始，层层递进。"
        if score["complex"] >= 3:
            return "complex", "real_time_tracking", "你现在进入实时追踪+多源交叉验证模式。"
...
{{
  ...
  "thinking_mode": "first_principles | creative_exploration | rigorous_verification | rapid_synthesis | real_time_tracking",
  "mode_prompt": "你现在进入【具体模式描述】模式：...",
  "confidence": 0-100,
  "reason": "一句话原因"
}}
```
...
"""
        ...
        try:
            ...
            thinking_mode = data.get("thinking_mode", "rapid_synthesis")
            mode_prompt = data.get("mode_prompt", "请以最优认知模式贡献。")

            logging.info(f"🧠 Echo思考模式已确定: {thinking_mode}")
            ...
            return fallback_category, "rapid_synthesis", "请以最优认知模式贡献。"
            # ================================================================
    ...
    def _solve_balanced(
        ...
        # 🔥 关键提升1：注入 Master Plan（移到外面，确保始终执行）
        plan = self._generate_detailed_plan(task, history)

        # 🔥 Echo自由Agent激活（顺序正确，使用 solve() 已计算的结果）
        selected_agents = self.agents[:3]  # 先定义
        thinking_mode = getattr(self, '_echo_thinking_mode', 'first_principles')
        mode_prompt = getattr(self, '_echo_mode_prompt', '你现在进入第一性原理拆解模式。')
        echo = self.create_echo_agent(thinking_mode, mode_prompt)
        selected_agents.append(echo)
        ...
        # Plan Validation Pass（保持原样）
        if plan and self.enable_adversarial_debate:
            improved_plan = self.agents[2].generate_response(
                [{"speaker": "System",
        ...
        tracker.checkpoint("4️⃣ 最终综合 + Verifier")
        return final_answer

    def _compress_history(self, history: List[Dict], max_tokens_approx: int = 18000) -> List[Dict]:
        """🔥 OPTIMIZED v4.2：更激进压缩 + 强制保留Master Plan + 自检报告"""
        if len(history) < 6:
            return history

        # 永久保留：最新Master Plan + 自检报告 + Primal记忆
        plan_msg = next((h for h in history if "Master Plan" in str(h.get("content", ""))), None)
        self_check_msg = next((h for h in history if "最终自检报告" in str(h.get("content", ""))), None)
        primal_msg = next((h for h in history if "Primal记忆" in str(h.get("content", ""))), None)

        # 保留最后4轮（更激进）
        compressed = [h for h in history if h["speaker"] in ["System", "User"]]
        if plan_msg:
            compressed.insert(0, plan_msg)
        if self_check_msg:
            compressed.append(self_check_msg)
        if primal_msg:
            compressed.insert(1, primal_msg)

        compressed.extend(history[-8:])  # 最后4轮（每轮约2条）

        # 超长时强制一键总结
        if len(str(compressed)) > max_tokens_approx * 3:
            short_summary = self.leader.generate_response(
                [{"speaker": "System", "content": "用400字以内总结以上全部历史，只保留关键决策、Master Plan阶段结论、文件链接和最终自检结果。"}]
                + compressed[-5:],
                0,
                force_non_stream=True,
                system_extra="只输出纯总结，不要任何标签。"
            )
            compressed = [
                {"speaker": "System", "content": f"📜 历史压缩总结（保留核心）：\n{short_summary}"}
            ]
